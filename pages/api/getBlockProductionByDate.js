...
export default async function getBlockProductionByDate (req, res) {
    const { date, "lines[]":lines } = req.query;
    const colRef = collection(database, 'efficiencies');
    
    const q = query(colRef, where("date", "==", new Date(date)),  where("lineNumber","in", lines));
    const querySnapshot = await getDocs(q);

    const tempAllLineEfficiency = [];
    querySnapshot.forEach((doc) => {
        const data = doc.data();
        const netProduction = Number(data["production"]) + Number(data["without"]) - Number(data["due"]) + Number(data["rejection"]);
