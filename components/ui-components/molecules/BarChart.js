...
const BarChartUI = ({data, dataKey, targetKey,  yDataKey}) => {
  ...
  return (
    <ResponsiveContainer width={data.length*60} height={200}>
      <ComposedChart  margin={{top:30, bottom:0, left:0, right:0}} data={data}>
        <Tooltip wrapperStyle={{zIndex: 1000}} content={CustomTooltip} />
        {/* <Bar barSize={10} dataKey={dataKey} fill="#0C9C00" opacity={0.5}> */}
        {/* <Bar barSize={10} dataKey={dataKey} fill="#0C9C00" opacity={0.5}
        >
        {data?.map((entry, index) => (
                    <Cell
                      // radius={5}
                      key={`cell-${index}`}
                     fill={entry.issue?"#FFAB00":"#0C9C00"}
                    />
                  ))}
        </Bar> */}
        <Bar 
        isAnimationActive={false}
              ...
              <YAxis axisLine={false} tickLine={false} dataKey={dataKey} fontSize={10} opacity={0.6} />
              <XAxis tickLine={false} dataKey={yDataKey} fontSize={10} opacity={0.6} />
